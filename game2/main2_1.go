...
var loaded = map[string]int{"red": 12, "green": 13, "blue": 14}
...
func Game2_1() {
	...
	for fileScanner.Scan() {
		if id := isPossible1(fileScanner.Text()); id > 0 {
			sum += id
		}
	}
...
func isPossible1(line string) int {
		...
		for _, cube := range strings.Split(strings.ReplaceAll(tirage, " ", ""), ",") {
			for i, b := range []byte(cube) {
				if b > '9' || b < '0' {
					...
					if count > loaded[colour] {
						fmt.Printf("Can't do game %d\n", id)
						return -1
					}
					break
				}
